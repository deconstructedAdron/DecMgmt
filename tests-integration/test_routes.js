 ...
 * HTTP Status codes: Located @ http://en.wikipedia.org/wiki/List_of_HTTP_status_codes
 ...
 The server has fulfilled a GET request for the resource, and the response is a representation of the result of one or more instance-manipulations applied to the current instance.[4]
 ...
 The requested resource is only available through a proxy, whose address is provided in the response. Many HTTP clients (such as Mozilla[8] and Internet Explorer) do not correctly handle responses with this status code, primarily for security reasons.[citation needed]
 306 Switch Proxy
 No longer used. Originally meant "Subsequent requests should use the specified proxy."[9]
 ...
 The origin server requires the request to be conditional. Intended to prevent "the 'lost update' problem, where a client GETs a resource's state, modifies it, and PUTs it back to the server, when meanwhile a third party has modified the state on the server, leading to a conflict."[12]
 429 Too Many Requests (RFC 6585)
 The user has sent too many requests in a given amount of time. Intended for use with rate limiting schemes.[12]
 431 Request Header Fields Too Large (RFC 6585)
 The server is unwilling to process the request because either an individual header field, or all the header fields collectively, are too large.[12]
 440 Login Timeout (Microsoft)
 A Microsoft extension. Indicates that your session has expired.[13]
 ...
 Defined in the internet draft "A New HTTP Status Code for Legally-restricted Resources".[16] Intended to be used when resource access is denied for legal reasons, e.g. censorship or government-mandated blocked access. A reference to the 1953 dystopian novel Fahrenheit 451, where books are outlawed.[17]
 451 Redirect (Microsoft)
 Used in Exchange ActiveSync if there either is a more efficient server to use or the server cannot access the users' mailbox.[18]
 The client is supposed to re-run the HTTP Autodiscovery protocol to find a better suited server.[19]
 494 Request Header Too Large (Nginx)
 Nginx internal code similar to 431 but it was introduced earlier in version 0.9.4 (on January the 21th, 2011).[20][original research?]
 ...
 Returned by ArcGIS for Server. A code of 498 indicates an expired or otherwise invalid token.[21]
 499 Client Closed Request (Nginx)
 Used in Nginx logs to indicate when the connection has been closed by client while the server is still processing its request, making server unable to send a status code back.[22]
 499 Token required (Esri)
 Returned by ArcGIS for Server. A code of 499 indicates that a token is required (if no token was submitted).[23]
 ...
 Transparent content negotiation for the request results in a circular reference.[24]
 507 Insufficient Storage (WebDAV; RFC 4918)
 The server is unable to store the representation needed to complete the request.[3]
 508 Loop Detected (WebDAV; RFC 5842)
 The server detected an infinite loop while processing the request (sent in lieu of 208 Already Reported).
 509 Bandwidth Limit Exceeded (Apache bw/limited extension)[25]
 This status code is not specified in any RFCs. Its use is unknown.
 510 Not Extended (RFC 2774)
 Further extensions to the request are required for the server to fulfill it.[26]
 511 Network Authentication Required (RFC 6585)
 The client needs to authenticate to gain network access. Intended for use by intercepting proxies used to control access to the network (e.g., "captive portals" used to require agreement to Terms of Service before granting full Internet access via a Wi-Fi hotspot).[12]
 ...
 This status code is not specified in any RFCs, but is used by Cloudflare's reverse proxies to indicate that the origin webserver refused the connection.[27]
 522 Connection timed out (Cloudflare)
 This status code is not specified in any RFCs, but is used by Cloudflare's reverse proxies to signal that a server connection timed out.[28]
